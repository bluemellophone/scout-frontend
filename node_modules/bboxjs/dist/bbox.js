var bboxjs = (function () {
  ...
  */
  ...
  function calculate_angle(pointa, pointb) {
    var xa, ya, xb, yb, x1, x2, y1, y2, x, y, angle; // Get the points and coordinates of extrema
    ...
    y2 = Math.max(ya, yb);
    x = x2 - x1;
    y = y2 - y1; // calculate the angle, preserving sign after arctangent
  ...
  jquery.fn.swapWith = function (that) {
    ...
    var $this = this;
    var $that = jquery(that); // create temporary placeholder

    var $temp = jquery('<div>'); // 3-step swap
  ...
  BBoxSelector = function () {
    function BBoxSelector(frame, options) {
      ...
      options.debug !== undefined || (options.debug = false); // Global attributes
      ...
      this.debug = []; // Create objects

      this.create_selector_elements(frame); // Set (update) selector mode

      this.update_mode(this.options.mode);
    }

    BBoxSelector.prototype.create_selector_elements = function (frame) {
      var selector_style, debug_style; // Define the image frame as an element

      this.elements.frame = frame; // Default style inherited by both selectors
      ...
      this.elements.rectangle = jquery('<div id="' + this.options.ids.rectangle + '"></div>');
      this.elements.rectangle.css(selector_style);
      this.elements.frame.append(this.elements.rectangle);
      this.elements.rectangle.hide(); // Create diagonal selector object

      this.elements.diagonal = jquery('<div id="' + this.options.ids.diagonal + '"></div>');
      this.elements.diagonal.css(selector_style);
      this.elements.frame.append(this.elements.diagonal);
      this.elements.diagonal.hide(); // Create the debug objects for visual purposes
      ...
      if (this.options.debug) {
        ...
        for (var index = 0; index < colors.length; index++) {
          this.debug.push(jquery('<div></div>'));
    ...
    BBoxSelector.prototype.check_boundaries = function (event) {
      ...
      point.y = Math.max(point.y, 0 - bounds.y.min); // Check upper boundaries
    ...
    BBoxSelector.prototype.start = function (event, subentry) {
      ...
      this.current_stage = 0; // Show selectors, as needed
      ...
      this.elements.diagonal.css(css_subentry); // Set global cursor mode and prevent highlight selection
    ...
    BBoxSelector.prototype.stage = function (event) {
      var finish;
      finish = false; // Used only for a multi-staging selection (i.e. diagonal)
    ...
    BBoxSelector.prototype.finish = function (event) {
      var data; // If there is a null event, don't do much
      ...
      this.elements.rectangle.hide();
      this.elements.diagonal.hide(); // Reset the global cursor mode and re-allow highlight selection
    ...
    BBoxSelector.prototype.get_selector_data = function (event) {
      var width, height, data, diff_x, diff_y; // Create the selector data object
      ...
      data.angles.theta = data.angles.origin; // Calculate the left, top, width, and height in pixel space
      ...
      height = this.elements.frame.height(); // Calculate the final values in percentage space
      ...
      data.percent.height = 100.0 * data.pixels.height / height; // Normalize the theta angle range to be between 0 and 2*PI
    ...
    BBoxSelector.prototype.refresh = function (event) {
      ...
      if (this.options.mode == 'rectangle') {
        css_theta = data.angles.theta; // Update the appearance of the rectangle selection using percentages
  ...
  commonjsGlobal.BBoxAnnotator = function () {
    function BBoxAnnotator(url, options) {
      ...
      this.debug = []; // Keep track of the current state of the annotator
    ...
    BBoxAnnotator.prototype.create_annotator_elements = function (url) {
      var bba, debug_style;
      bba = this; // Define the container

      this.elements.container = jquery('#' + this.options.ids.container);
      ...
      this.elements.frame = jquery('<div class="ia-bbox-annotator-frame"></div>');
      ...
      this.elements.container.append(this.elements.frame); // Create the debug objects for visual purposes
      ...
      this.elements.image.src = url; // Create events to load the rest of the data, but only if the image loads
      ...
      this.elements.image.onload = function () {
        ...
        bba.bbs = new BBoxSelector(bba.elements.frame, bba.options); // This is the last stage of the constructor's initialization

        bba.register_global_key_bindings();
        bba.register_global_event_bindings(); // Resize the image to the window

        bba.resize(); // Trigger onload
    ...
    BBoxAnnotator.prototype.register_global_key_bindings = function (selector, options) {
      ...
      jquery(window).keydown(function (event) {
        ...
        key = event.which; // Shift key to zoom (un-zoom even if hotkeys are disabled)
        ...
        if (bba.options.hotkeys.enabled) {
          ...
          if (bba.options.hotkeys.focus.indexOf(key) != -1) {
            if (bba.state.hover != null) {
              ...
              bba.focus_entry(bba.state.focus); // If no longer hovering, hide entries
    ...
    BBoxAnnotator.prototype.register_global_event_bindings = function (selector, options) {
      var bba;
      bba = this; // Resize the container dynamically, as needed, when the window resizes
      ...
      jquery(window).mousemove(function (event) {
        if (bba.state.mode == 'selector') {
          ...
          if (bba.state.which == 3) {
            ...
            entry = bba.entries[bba.state.hover]; // VERSION 1

            resize_handle = element.resize[entry.closest];
            bba.resize_start(resize_handle, event); // VERSION 2
            // bba.anchor_update(event, entry.closest)
          } else if (bba.state.hover != null) {
            bba.state.mode = 'drag'; // Hide the close and rotate buttons

            element = bba.elements.entries[bba.state.hover];

            if (bba.state.focus == null || bba.state.focus != bba.state.hover) {
              // Set the other entries to be transparent
              bba.entries_style_transparent(bba.options.colors.transparent, bba.state.hover); // If we have a parent, do not hide
      ...
      jquery(window).mouseup(function (event) {
        if (event.which == 1) {
          if (bba.state.mode == 'free' && bba.state.inside) {
            ...
            var element; // Set the other entries to be transparent
    ...
    BBoxAnnotator.prototype.resize = function () {
      ...
      margin = 5; // Get the proportions of the image and
      ...
      for (var index = 0; index < this.entries.length; index++) {
        entry = this.entries[index]; // We only need to update the assignments for parents, which will update all of their sub-entries
    ...
    BBoxAnnotator.prototype.delete_entry = function (index, parent_index) {
      ...
      entry = this.entries.splice(index, 1); // Perform the exact same operation on the indices

      indices.splice(index, 1); // Detatch the bbox HTML elements at index
    ...
    BBoxAnnotator.prototype.add_entry = function (entry, notify) {
      var invalid, width, height, element, css_no_select, css_margin, css_handles, configurations, handle, index; // Check the notify boolean variable

      notify !== undefined || (notify = false); // Check the required bounding box location dimensions are specified
      ...
      console.log(entry); // Create the holder element
      ...
      element.bbox = jquery('<div class="' + this.options.classes.bbox + '"></div>');
      element.bbox.css({
        position: 'absolute',
      ...
      this.elements.frame.append(element.bbox); // If there is a parent add a line for assigning sub-entry

      if (entry.parent != null) {
        // Create diagonal selector object
        element.assignment = jquery('<div id="' + this.options.classes.assignment + '"></div>');
      ...
      element.label = jquery('<div class="' + this.options.classes.label + '"></div>');
      element.label.css({
        overflow: 'visible',
      ...
      element.bbox.append(element.label); // Add the close button to the bbox

      element.close = jquery('<div class="' + this.options.classes.close + '"></div>');
      element.close.css({
        position: 'absolute',
        top: '5px',
        right: '5px',
      ...
      element.bbox.append(element.close); // Add the X to the close button, which we do not need to keep track of
      ...
      element.close.append(temp); // Hide close button if diabled
      ...
      element.rotate = jquery('<div class="' + this.options.classes.rotate + '"></div>');
      ...
      element.bbox.append(element.rotate); // Resize CSS for all handles
      ...
      for (var key in configurations) {
        handle = jquery('<div id="' + this.options.ids.resize + '-' + key + '" class="' + this.options.classes.resize + '"></div>');
      ...
      this.register_entry_event_bindings(element); // Add the entry JSON and also the jQuery elements

      this.entries.push(entry);
      this.elements.entries.push(element); // Set the label for the entry
      ...
      if (this.state.focus == null) {
        ...
        this.hover_entry(null); // Set the appropriate visibility

        this.entries_style_visible();
      } // Update the assignment line, if needed


      this.assignment_entry(index); // notify on adding entry from selection
    ...
    BBoxAnnotator.prototype.hover_entry = function (index) {
      var entry; // Always update the state for the hover2 index

      this.state.hover2 = index; // Do not update the hover entry if currently selecting
      ...
      if (this.state.focus == null) {
        ...
        if (index != null) {
          this.entry_style_hover(index); // Show all sub-entries
    ...
    BBoxAnnotator.prototype.focus_entry = function (index) {
      ...
      if (this.state.focus == null) {
        ...
        entry = this.entries[index]; // Set the style of the focused entry

        this.entry_style_focus(index); // Hide all the rest of the bounding boxes

        this.entries_style_hidden(index); // Show the required sub-entries as well

        this.subentries_style_visible(index); // If there is a subentry, hover it when focusing
        ...
        if (index != this.state.focus) {
          if (this.state.focus2 == null) {
            var css_transparent;
            this.state.focus2 = index; // Set the style of the focused entry

            this.entry_style_focus(index); // Make the other sub-entries semi-transparent
            ...
            this.state.focus2 = null;
            this.state.hover = this.state.hover2; // Make the other sub-entries semi-transparent
          ...
          this.state.focus = null; // Set the style of the un-focused entry back to hover

          this.hover_entry(this.state.hover); // Un-hide all the rest of the bounding boxes

          this.entries_style_visible(index); // Show the required sub-entries as well
    ...
    BBoxAnnotator.prototype.anchor_update = function (event, handle) {
      ...
      offset = this.elements.container.offset(); // We need to un-rotate the box before we can get a correct position

      this.rotate_element(element.bbox, 0.0); // Get the accurate position from jQuery

      position = element.bbox.position(); // Update the cursor and element anchors
      ...
      this.rotate_element(element.bbox, entry.angles.theta); // Get resize handles locations
    ...
    BBoxAnnotator.prototype.label_entry = function (index, label) {
      ...
      entry = this.entries[index];
      element = this.elements.entries[index]; // Set the label in the entry's metadata

      entry.label = label; // Update the element's text box with the new label
    ...
    BBoxAnnotator.prototype.get_metadata = function () {
      var index, entry;
      index = this.state.hover; // Do not update if there is not a hover entry
    ...
    BBoxAnnotator.prototype.anchor_entry = function (event) {
      ...
      index = this.state.hover; // Do not update if there is a focused entry
      ...
      for (var key in element.resize) {
        ...
        dist = Math.sqrt(Math.pow(diff_x, 2) + Math.pow(diff_y, 2)); // Get the closest
    ...
    BBoxAnnotator.prototype.assignment_entry = function (index) {
      var index, entry, element, parent, width, height, centers, delta, hypo, angle; // Do not update if there is not a hover entry
      ...
      entry = this.entries[index];
      element = this.elements.entries[index]; // Check to make sure this entry has a parent, if not move all sub-entries
      ...
      var height = this.elements.frame.height(); // Compute centers
    ...
    BBoxAnnotator.prototype.move_entry = function (event) {
      ...
      entry.percent.top = 100.0 * entry.pixels.top / height; // // If the shift is held, move parts as well
      // if(event.shiftKey) {
      // }
      ...
      this.assignment_entry(index); // Update the entries
    ...
    BBoxAnnotator.prototype.move_entry_correction = function (event, correction) {
      ...
      event.preventDefault();
      entry = this.entries[index];
      element = this.elements.entries[index]; // We need to un-rotate the box before we can get a correct position

      this.rotate_element(element.bbox, 0.0); // Get the accurate position from jQuery

      position = element.bbox.position(); // Re-rotate the box now that we have accurate coordinates
    ...
    BBoxAnnotator.prototype.rotate_entry = function (event) {
      ...
      correction = this.state.anchors.element.theta + delta; // If the Shift key is pressed, block to steps
      ...
      this.rotate_element(element.bbox, entry.angles.theta); // Update the assignments, if needed
    ...
    BBoxAnnotator.prototype.orient_entry = function (event, direction, index) {
      ...
      index !== undefined || (index = this.state.hover); // Do not update if there is not a hover entry
      ...
      this.rotate_element(element.bbox, entry.angles.theta); // Update the entries
    ...
    BBoxAnnotator.prototype.background_entry = function (event, index, subentry) {
      ...
      subentry !== undefined || (subentry = true); // Do not update if there is not an index
      ...
      if (entry.parent == null) {
        ...
        for (var index_entry = 0; index_entry < this.entries.length; index_entry++) {
          ...
          if (parent != null && parent == this.state.focus) {
            indices.push(index_entry); // We want to get only the subentries before the indexed subentry
        ...
        if (indices.length > 1) {
          ...
          index_penultimate = indices[indices.length - 2]; // Get last item
          ...
          element_penultimate = this.elements.entries[index_penultimate]; // Swap last two items in entries and elements
          ...
          this.elements.entries[index_penultimate] = element_last; // Swap last two DOM elements

          element_last.bbox.swapWith(element_penultimate.bbox); // Continue, recursively
      ...
      this.entries.unshift(entry[0]); // Perform the exact same operation on the indices

      entry = indices.splice(index, 1);
      indices.unshift(entry[0]); // Move the element to the start of the list

      element = this.elements.entries.splice(index, 1);
      this.elements.entries.unshift(element[0]); // Prepend the element to the frame

      holder = element[0].bbox.detach();
      this.elements.frame.prepend(holder); // Now we need to fix the sub-entry parent indices
    ...
    BBoxAnnotator.prototype.resize_start = function (resize_element, event) {
      ...
      handle = resize_element.prop(bba.options.props.resize); // Set the other entries to be transparent
      ...
      bba.state.orientation = Math.floor(bba.state.orientation); // Set the border-top based on the correct orientation
      ...
      for (var index = bba.state.orientation; index > 0; index--) {
        bba.orient_entry(event, 'left'); // Update the handle
    ...
    BBoxAnnotator.prototype.resize_finish = function (event) {
      ...
      if (bba.state.hover != null) {
        entry = bba.entries[bba.state.hover];
        element = bba.elements.entries[bba.state.hover]; // Set the element border with the correct orientation
    ...
    BBoxAnnotator.prototype.resize_entry = function (event) {
      ...
      var offset, cursor, delta, theta, hypo;
      var angle, diff_angle, diff, components, offset;
      var width, height, position, position_resize, temp, correction;
      index = this.state.hover;
      handle = this.state.anchors.element.handle; // Do not update if there is not a hover entry
      ...
      height = this.state.anchors.element.height + offset.height; // If the width and height of the new location is negative
      ...
      entry.pixels.top -= correction.y; // Calculate the final values in percentage space
    ...
    BBoxAnnotator.prototype.selector_start = function (event) {
      ...
      this.hover_entry(null); // Make the other entries semi-transparent

      this.entries_style_transparent(this.options.colors.transparent); // If we have a parent, do not hide
      ...
      this.state.mode = 'selector'; // notify on adding entry from selection
    ...
    BBoxAnnotator.prototype.selector_stage = function (event) {
      var finish; // Stage the BBoxSelector
    ...
    BBoxAnnotator.prototype.selector_cancel = function (event) {
      if (this.state.mode == 'selector') {
        ...
        this.bbs.finish(event); // Release the mode to "free"

        this.state.mode = 'free'; // Make the other entries semi-transparent
    ...
    BBoxAnnotator.prototype.selector_finish = function (event) {
      var data, invalid, index; // If we are in the "selector" mode, finish the BBoxSelector and add the entry
      ...
      if (this.state.mode == 'selector') {
        ...
        invalid = invalid || data.pixels.height <= this.options.limits.entry.height; // Add the returned entry data reported by the BBoxSelector as a new entry
    ...
    BBoxAnnotator.prototype.entries_style_transparent = function (opacity, skip) {
      var css_transparent; // Check the skip index
    ...
    BBoxAnnotator.prototype.update_subentries_options = function () {
      if (this.state.focus == null) {
        ...
        for (var index = 0; index < this.elements.entries.length; index++) {
          ...
          if (entry.parent == this.state.focus) {
            element = this.elements.entries[index]; // Check if we are an entry or sub-entry

            if (this.options.subentries.visible == false) {
              element.bbox.hide(); // Set the assignment

              if (element.assignment != null) {
                element.assignment.hide();
              }
            } else {
              element.bbox.show(); // Set the assignment
    ...
    BBoxAnnotator.prototype.entries_style_visible = function (skip) {
      var entry, element; // Establish the desired parent, if wanted

      skip !== undefined || (skip = null); // Make the other entries semi-transparent
    ...
    BBoxAnnotator.prototype.subentries_style_visible = function (parent) {
      var entry, element; // Make the other entries semi-transparent

      for (var index = 0; index < this.elements.entries.length; index++) {
        entry = this.entries[index];
        element = this.elements.entries[index]; // If not a sub-entry, skip
    ...
    BBoxAnnotator.prototype.entries_style_hidden = function (skip) {
      var element; // Establish the desired parent, if wanted
    ...
    BBoxAnnotator.prototype.entry_style_default = function (index) {
      var entry, element, color_background, color_text; // Get the appropriate entry element

      entry = this.entries[index];
      element = this.elements.entries[index]; // Update element CSS

      color_background = entry.highlighted ? this.options.colors.highlighted : this.options.colors["default"];
      element.bbox.css(_defineProperty({
        'border-color': color_background,
        opacity: '0.8',
        cursor: 'crosshair',
        'background-color': 'rgba(0, 0, 0, 0.0)'
      }, "background-color", 'rgba(0, 0, 0, 0.0)')); // Update the assignment CSS
    ...
    BBoxAnnotator.prototype.entry_style_hover = function (index) {
      var entry, element; // Get the appropriate entry element
    ...
    BBoxAnnotator.prototype.entry_style_focus = function (index) {
      var element; // Get the appropriate entry element
    ...
    BBoxAnnotator.prototype.register_entry_event_bindings = function (element) {
      var bba;
      bba = this; // Register events for when the box is hovered and unhovered

      element.bbox.hover(function () {
        var index;
        index = jquery(this).prevAll('.' + bba.options.classes.bbox).length;
      ...
      if (element.assignment != null) {
        element.assignment.hover(function () {
          var index;
          index = jquery(this).prevAll('.' + bba.options.classes.bbox).length - 1;
          bba.hover_entry(index);
        }, function () {
          bba.hover_entry(null);
        });
      ...
      element.close.mouseup(function (event) {
        var index; // Find the parent bbox of the close button and the bbox's index

        parent = jquery(this).parent('.' + bba.options.classes.bbox);
        index = parent.prevAll('.' + bba.options.classes.bbox).length; // We want to prevent the selector mode from firing from "free"

        bba.state.mode = 'close'; // Delete the entry from the annotator
      ...
      element.rotate.mousedown(function (event) {
        ...
        index = parent.prevAll('.' + bba.options.classes.bbox).length;
        entry = bba.entries[index];
        element = bba.elements.entries[index]; // We want to prevent the selector mode from firing from "free"

        bba.state.mode = 'rotate'; // Set the transparency of other entries

        bba.entries_style_transparent(bba.options.colors.transparent, index); // If the
      ...
      for (var key in element.resize) {
        element.resize[key].mousedown(function (event) {
          if (event.which == 1) {
            ...
            bba.state.mode = 'resize'; // Start the resizing

            bba.resize_start(jquery(this), event);
          }
        });
      }
    };
